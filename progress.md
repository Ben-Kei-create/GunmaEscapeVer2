...
- 2026-04-05: 監査とスモークテストの要点
  - `README.md` と `progress.md`、`js/battle.js`, `js/ritual_battles.js`, `js/story.js`, `js/event.js` を突き合わせ、次の整理を確認。
  - `通常戦闘の群れ演出 / 敵ごとの差別化`: 複数敵、背景差分、導入ラベル、戦果余韻までは実装済み。敵意図表示や群れ固有の連携は未完。
  - `仲間固有コマンド / 同行アニメ / 支援発動演出`: 同行枠、支援ボーナス、戦果ログはあるが、固有コマンドと戦闘中の発動演出は未実装。
  - `各章の環境ストーリーテリング`: 前橋 / 高崎 / 下仁田 / 富岡など一部は実装済みだが、全章にはまだ広がっていない。
  - `儀式戦の余韻演出 / 音 / 無音`: 儀式戦ごとの余韻テキストと一部SEは実装済み。章固有の静けさ演出は未完。
  - `event.js / story.js 整理`: 章導入の補強は進んでいるが、二重構造そのものの整理は未完。
  - `終章の奉納UI`: `js/ritual_battles.js` に `offering` 用スロット土台はあるが、実戦投入はまだ。
  - セーブはタイトル / 記録の牧師 / マップ遷移時の autosave(slot1) が中心で、手動セーブ導線は薄い。
- 2026-04-05: スモークテスト
...
- 2026-04-10: 哀愁・慈悲寄りの新規わざ10種を追加し、習得導線と戦闘効果も接続。
  - `js/skills.js`
    - `弔い唄 / 繭ごもり / 濡れ羽 / 水鏡 / 捧げ火 / ほつれ指 / 土の音 / 涙傘 / 風送り / 黄泉返し` を追加。
    - ランク習得順へ後半10種を追加し、`shimonita_packer / tomioka_weaver / yubatake_guardian / haruna_lake_beast / oze_mud_wraith / echo_guardian` の勝利ひらめきも設定。
  - `js/battle.js`
    - 新技ごとの効果を実装。敵攻撃低下、状態異常防御、致死耐え、水鏡による無効化、儀式ゲージ低下、敵味方同時回復などを既存戦闘へ接続。
    - 新しい状態表示 `護 / 低 / 鏡 / 土 / 境 / 攻↓` を戦闘HUDへ追加。
    - `濡れ羽` は次の高い目を低く抑え、`土の音` は次の一投へ小回復と返しを添えるようにして、儀式戦でも使い道が見えるように調整。
- 2026-04-05: すぐ効く改善を実装。
  - `js/ui.js`
    - フィールドメニューの `記録と設定` に `記録帳` を追加し、探索中のどこからでも記録帳を開けるようにした。
    - 同セクションの詳細欄で、`slot1` の続き先を確認できるようにし、今どこへ戻る記録なのか把握しやすくした。
  - `js/main.js`
    - フィールドメニューから `Game.SaveMenu` を開く分岐を追加。
  - `js/battle.js`
    - 敵ごとの読み分けを助けるため、通常戦と群れ戦の敵へ `気配タグ` と短い `意図テキスト` を表示。
    - 群れ戦では敵ごとの色味を帯びた足元影と識別チップを出し、見分けやすさを上げた。
- 2026-04-05: 改修後の確認
  - `node --check js/ui.js js/main.js js/battle.js` 通過。
  - `WEB_GAME_CLIENT` でタイトル画面と通常戦 (`?debugBattle=roadsideBandit`) のスクリーンショットを再取得し、通常敵の `気配タグ + 意図` が乗っていることを確認。
  - headless Playwright で `field menu -> section 4 -> Enter` を通し、`mode: 'save'` へ遷移することを確認。
  - 同テストで `group_battle` と `ritual_battle` を再取得し、新規 console error / pageerror は 0 件。
- 2026-04-05: 仲間固有コマンドと支援発動演出の初回実装。
  - `js/player.js`
    - `アカギ / 山川 / 古谷` に `supportCommand` を追加し、固有支援名・短い説明・効果量を定義。
  - `js/battle.js`
    - 通常メニューへ `支援` を追加し、同行中の仲間ごとに固有コマンドを選べる `supportMenu` フェーズを実装。
    - 各仲間の支援は 1 戦 1 回に制限し、同行パネルへ `支援可 / 使用済` を表示。
    - 支援発動時は仲間色のオーバーレイを挟み、粒子・SE・テキストで「誰が入ったか」を視覚化。
    - 効果は `アカギ=敵賽鈍化+見切り`, `山川=防御強化+ward`, `古谷=攻撃強化+次投補正` として接続。
- 2026-04-05: 仲間支援の確認
  - `node --check js/player.js js/battle.js` 通過。
  - Playwright MCP で `?debugBattle=roadsideBandit` に `アカギ / 山川 / 古谷` を同行させ、通常戦メニューに `支援` が増えることを確認。
  - 同テストで `アカギ` 発動直後に `supportAction`, `slow_roll`, `enemy_roll_slow` が戦闘スナップショットへ反映されることを確認。
  - 続けて `山川` 発動直後に `defense_up`, `ward`, `supportAction` が反映され、`アカギ` が `disabled: true` になることを確認。
  - 上記検証中の新規 console error は 0 件。
